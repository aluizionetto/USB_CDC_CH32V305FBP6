...
uint8_t RCC_Configuration( void )
{
	...
	//RCC_APB1PeriphClockCmd( RCC_APB1Periph_USART2, ENABLE );  //não usa serial
...
void UART2_USB_Init( void )
{
	...
	//UART2_Init( 0, baudrate, stopbits, parity );  //não utiliza USART2
...
void USB_CDC_Send(uint8_t *data, uint32_t len) {

	//copia dados para buffer
	memcpy(UART2_Rx_Buf,data,len);

	//ativa interrupção de dma para envio de dados
	NVIC_DisableIRQ( USBHS_IRQn );
	NVIC_DisableIRQ( USBHS_IRQn );
	Uart.Rx_TimeOut  = 0;
	Uart.USB_Up_IngFlag = 0x01;
	Uart.USB_Up_TimeOut = 0x00;
	USBHSD->UEP2_TX_DMA = (uint32_t)(uint8_t *)UART2_Rx_Buf;
...
uint16_t USB_CDC_RxAvailable(void) {
	return Uart.Tx_PackLen[Uart.Tx_DealNum];
}

void USB_CDC_Rx(uint8_t *data, uint32_t len) {
	...
	count = (len < Uart.Tx_PackLen[ Uart.Tx_DealNum ])?len:Uart.Tx_PackLen[ Uart.Tx_DealNum ];
	memcpy(data,&UART2_Tx_Buf[ Uart.Tx_CurPackPtr],count);
	Uart.Tx_PackLen[ Uart.Tx_DealNum ] = 0x0000;
	Uart.Tx_DealNum++;
	if( Uart.Tx_DealNum >= DEF_UARTx_TX_BUF_NUM_MAX )
	{
